...
import { Users, CircleParking, Building2, Star } from "lucide-react";
...
const stats = [
  ...
  {
    icon: <CircleParking size={22} className="text-amber-500" />,
    end: 10,
    suffix: "k+",
    label: "Parking Spots",
  },
  {
    icon: <Building2 size={22} className="text-amber-500" />,
    end: 50,
    suffix: "+",
...
export default function Hero() {
  const lines = ["Find Parking.", "Schedule Laundry.", "Simplify Your day."];
  ...
  useEffect(() => {
    ...
    const animateCounters = () => {
      ...
      const timer = setInterval(() => {
        ...
        setCounts(stats.map((stat) => Math.floor(stat.end * progress)));

        if (progress === 1) {
          clearInterval(timer);
          setTimeout(() => animateCounters(), pause);
        }
      }, 30);
    };
          ...
          <div className="text-white animate-fade-up">
            ...
            <div className="mt-8 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-300">Join thousands of happy drivers</p>
              </div>
            </div>
          </div>
